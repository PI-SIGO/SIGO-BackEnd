...
namespace SIGO.Controllers
{
    ...
    public sealed class PecaController : ControllerBase
    {
            ...
            [FromQuery] PaginationRequest? pagination = null)
        {
            ...
            return Ok(PagedResponse<PecaDTO>.Create(pieces, pagination));
        }
        ...
        public async Task<ActionResult<PecaDTO>> Get(int id)
        {
            var piece = _currentUserService.IsInRole(SystemRoles.Admin)
                ...
                : await _pecaService.GetByIdForOficina(id, RequireOfficeId());
            return piece is null
                ? this.ApiProblem(StatusCodes.Status404NotFound, "Peça não encontrada.")
                : Ok(piece);
        }

        [HttpPost]
        public async Task<ActionResult<PecaDTO>> Post([FromBody] PecaDTO request)
        {
            if (_currentUserService.IsInRole(SystemRoles.Admin))
                await _pecaService.Create(request);
            else
                await _pecaService.CreateForOficina(request, RequireOfficeId());

            return Created($"/api/v1/pecas/{request.Id}", request);
        }

        [HttpPut("{id:int}")]
        public async Task<ActionResult<PecaDTO>> Put(int id, [FromBody] PecaDTO request)
        {
            if (_currentUserService.IsInRole(SystemRoles.Admin))
                await _pecaService.Update(request, id);
            else
                await _pecaService.UpdateForOficina(request, id, RequireOfficeId());
